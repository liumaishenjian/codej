...
| TASK-05 | Task Tool、协议与 TUI | 四个 Model Tool 在 Plan planning 与批准 execution 中持续可用并统一经过 Pipeline/Hook/Permission；两阶段共享同一 Session Board，不从 Markdown seed 或按标题匹配 Task。简单 `task_update` 接受 subject/description/active_form/status/owner/dependency 字段，CAS/claim/phase mutation 由宿主内部处理；Session-local root 的 owner 标签规范化为当前 capability actor，child 仍精确校验。strict stdio codec 接受无参数 `/tasks`；活动 Run 的最多两行进度固定在 Model 状态之后、Tool 历史之前且不参与 flex shrink。真实 Java→stdio→Ink E2E 通过 Ink 审批建立 Run correlation，在规划期创建五个中文 Task，批准执行复用同一 task-1..5 逐项显示活动、勾选和 completed strikethrough/dim；隔离 Workspace 从不存在开始生成并由独立进程重开验证 18×7=126 条 OpenXML XLSX。真实 Provider 冒烟另验证三项中文 Task、执行期 2/3 活动投影、最终 3/3 与真实文本产物 | L2 | S15 | AUTH-01/ADR-088 |
