...
| CLI-04 | Tool 进度与输出 | 有序 Agent Event → 同 Run/回合/同类相邻成功搜索聚合且失败独立、异构活动 8 行有界摘要、默认折叠结构化 stdout/stderr、运行中及最近历史 Tool 详情选择；ADR-096 由实际执行器同源投影有/无审批命令的完整 command/shell/cwd，旧宿主缺字段不猜测；ADR-087 白名单投影参数修正/策略变化，连续失败不再只显示次数 | L2 | S02/S03/S15 | REF-02/AUTH-01/CODEX-0.147 |
...
| CLI-09 | 多行、历史、补全 | `ComposerState` grapheme 光标、视觉多行/viewport、完整编辑键、History/Completion 优先级；删除用户可见行列诊断，主屏历史不裁剪；8,192 仅限可见结构，约 1 MiB 无损大 Paste 经 stdio v0 原子分块提交后仍受 256K Token Context pipeline 权威治理。ADR-097 为定向长 Plan 增加初始化 capability 协商：旧 Host 未确认 `directedChunkInputV1` 时长 Plan 在写入前失败关闭，不降级成普通 Run；未协商普通长 Run 继续旧分片，request/session/inputId/digest/chunk/replay 绑定不变 | L2 | S08/S15 | REF-02/AUTH-01 |
...
| TOOL-10 | Run Command | 固定 Shell/Workspace/审批/timeout；ADR-096 由实际 backend 产生稳定 shell、workspace-relative `.` 与完整命令显示元组。ADR-097 的 Windows 执行边缘使用固定短小 UTF-16LE `-EncodedCommand` launcher，已批准正文只进入本次 Process environment，执行前清除且不被孙进程继承；`-OutputFormat Text`、Console UTF-8 与 `$OutputEncoding` 保持纯文本 Unicode stdout/stderr，审批/Permission selector/cwd/timeout/cancel 不变。PowerShell 7 已验证 8192 supplementary code points、Host/raw 混合中文、`Write-Error`、native failure、显式 exit 与隐私安全诊断；真实生产入口已验证混合中文/emoji、纯文本中文错误、预期 exit 7、正确非成功正文和下一轮。Windows PowerShell 5.1 未实机验证 | L1 | S04/S15 | REF-02/AUTH-01 |
