...
| HEAD | `e51e6b5062fbab46881137342a2bff84bfc05645` |
| 实现状态 | 新 TUI 和近期修复包含大量未提交修改；仅检出 HEAD 不会获得这些实现 |
...
其他工具接手时，必须先确认拿到**完整工作树，包括未跟踪文件**。禁止直接重置工作树或整包迁入旧重构实现。交接材料不得包含本地凭证、用户会话或构建缓存。实施前遵循仓库 AGENTS.md 的必读顺序及第 5.3 节。
